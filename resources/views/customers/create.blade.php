...
<div class="container-fluid py-4">
    ...
    <div class="shad-page-header">
        ...
        <div class="d-flex gap-2">
            {{-- Template Button --}}
            <a href="{{ route('customers.template') }}" class="shad-btn shad-btn-ghost">
            ...
            {{-- Export Button --}}
            <form action="{{ route('export.finance-customers') }}" method="POST" style="display: inline;">
                @csrf
                ...
                <div class="table-responsive">
                    <table class="shad-table">
                        ...
                        <tbody>
                            ...
                            @php $displayName = $customer->customerName ?: $customer->name ?: 'N/A'; @endphp
                            <tr class="customer-row" data-customer-id="{{ $customer->id }}" data-customer-name="{{ strtolower($displayName) }}">
                                <td class="text-center" style="color: var(--slate-500);">{{ $customers->firstItem() + $loop->index }}</td>
            ...
            <div class="modal-header" style="background: #f0fdf4; border-bottom: 1px solid #bbf7d0;">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <div>
                        ...
                        <small style="color: #16a34a;">{{ count($importedRows) }} customer berjaya diimport ke sistem</small>
                    </div>
                </div>
                    ...
                    <table style="width: 100%; border-collapse: collapse; font-size: 0.8125rem;">
                        ...
                        <tbody>
                            ...
                            <tr style="{{ $loop->odd ? 'background:#fff;' : 'background:#f9fafb;' }} border-bottom: 1px solid #f3f4f6;">
                                ...
                                <td style="padding: 0.5rem 0.75rem; font-weight: 600; color: #111827;">{{ $row['customerName'] }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        iziToast.error({ title: 'Error', message: {!! json_encode(session('error')) !!}, position: 'topRight', timeout: 3000 });
    });
</script>
@endif
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.delete-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            document.getElementById('deleteForm').action = '{{ route("customers.destroy", ":id") }}'.replace(':id', this.dataset.id);
