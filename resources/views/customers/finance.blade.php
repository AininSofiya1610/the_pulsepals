...
<div class="container-fluid py-4">
    ...
    <div class="shad-page-header">
        ...
        <div class="d-flex gap-2">
            {{-- Template Button --}}
            <a href="{{ route('customers.finance.template') }}" class="shad-btn shad-btn-ghost">
            ...
            {{-- Export Button --}}
            <form action="{{ route('export.customer-finance') }}" method="POST" style="display: inline;">
                @csrf
    ...
    <div class="shad-card mb-4">
        <div class="shad-card-body py-3">
            <form action="{{ route('customers.finance') }}" method="GET" class="d-flex align-items-center flex-wrap gap-2">
                <div class="d-flex align-items-center gap-2">
                    <label class="shad-label mb-0">Search:</label>
                    <input type="text" name="search" class="shad-input" style="width: 180px;" value="{{ $search }}" placeholder="Invoice, Customer...">
                </div>
                <div class="d-flex align-items-center gap-2">
                    <label class="shad-label mb-0">From:</label>
                    <input type="date" name="start_date" class="shad-input date-dmy" style="width: auto;" value="{{ $startDate }}">
                </div>
                <div class="d-flex align-items-center gap-2">
                    <label class="shad-label mb-0">To:</label>
                    <input type="date" name="end_date" class="shad-input date-dmy" style="width: auto;" value="{{ $endDate }}">
                </div>
                <button type="submit" class="shad-btn shad-btn-primary shad-btn-sm">Filter</button>
                <a href="{{ route('customers.finance') }}" class="shad-btn shad-btn-ghost shad-btn-sm">Clear</a>
            </form>
        </div>
    </div>
    ...
    <div class="shad-card">
        ...
        <div class="shad-card-body p-0">
            @if(count($groupedInvoices) > 0)
                @foreach($groupedInvoices as $year => $months)
                    <!-- Year Section -->
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <span class="shad-badge shad-badge-primary" style="font-size: 0.875rem;">
                                        {{ collect($months)->sum('count') }} invoices
                                    </span>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <div class="d-flex gap-4" style="font-size: 0.875rem;">
                                            <span style="color: var(--slate-600);">
                                                <strong style="color: var(--slate-800);">RM {{ number_format($monthData['total_invoice'], 2) }}</strong>
                                            </span>
                                            <span style="color: #22c55e;">
                                                <i class="fas fa-arrow-down" style="font-size: 0.7rem;"></i>
                                                RM {{ number_format($monthData['total_received'], 2) }}
                                            </span>
                                ...
                                <div id="month-{{ $year }}-{{ $monthNum }}" class="collapse {{ $isCurrentMonth ? 'show' : '' }}">
                                    <div class="table-responsive">
                                        <table class="shad-table">
                                            ...
                                            <tbody>
                                                @foreach($monthData['invoices'] as $finance)
                                                @php
                                                ...
                                                @endphp
                                                <tr class="finance-row" style="{{ ($balance > 0 && \Carbon\Carbon::parse($finance->due_date)->isPast()) ? 'background-color: #fff5f5;' : '' }}">
                                                    <td class="text-center" style="color: var(--slate-500);">{{ $loop->iteration }}</td>
                                                    <td style="font-weight: 600; color: var(--slate-800);">{{ $finance->invoice_no }}</td>
                                                    ...
                                                    <td class="text-right">RM {{ number_format($finance->amount, 2) }}</td>
                                                    ...
                                                    <td>
                                                        <div class="shad-actions justify-content-end">
                                                            <button class="shad-btn shad-btn-ghost shad-btn-sm view-btn" data-id="{{ $finance->id }}" title="View">
                                                                <i class="fas fa-eye"></i>
                                                            </button>
                                                            <button class="shad-btn shad-btn-ghost shad-btn-sm delete-btn" data-id="{{ $finance->id }}" data-invoice="{{ $finance->invoice_no }}" style="color: #dc2626;" title="Delete">
...
<div class="modal fade shad-modal" id="importModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('customers.finance.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
...
<div class="modal fade shad-modal" id="invoiceModal" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('customers.finance.store') }}" method="POST" id="invoiceForm">
                @csrf
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="customer_name" class="shad-select" required>
                                ...
                                @foreach($customers as $customer)
                                    <option value="{{ $customer->name ?: $customer->customerName }}">{{ $customer->name ?: $customer->customerName }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
            ...
            <div class="modal-body">
                ...
                <div id="paymentFormSection" style="display: none;">
                    <div class="shad-card mb-3" style="border-left: 3px solid #22c55e;">
                        <div class="shad-card-body">
                            <form action="{{ route('customers.finance.payment.store') }}" method="POST" id="paymentForm">
                                @csrf
                    ...
                    <table style="width: 100%; border-collapse: collapse; font-size: 0.8125rem;">
                        ...
                        <tbody>
                            ...
                            <tr style="{{ $loop->odd ? 'background:#fff;' : 'background:#f9fafb;' }} border-bottom: 1px solid #f3f4f6;">
                                ...
                                <td style="padding: 0.5rem 0.75rem; color: #374151;">{{ $row['customer_name'] }}</td>
                                <td style="padding: 0.5rem 0.75rem; color: #374151;">{{ $row['invoice_date'] }}</td>
                                <td style="padding: 0.5rem 0.75rem; text-align: right; color: #111827; font-weight: 600;">RM {{ $row['amount'] }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
