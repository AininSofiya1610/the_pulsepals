...
<div class="container-fluid py-4">
    ...
    <div class="shad-page-header">
        ...
        <div class="d-flex gap-2">
            <form action="{{ route('export.leads') }}" method="POST" style="display: inline;">
                @csrf
            ...
            <a href="{{ route('leads.create') }}" class="shad-btn shad-btn-primary">
    ...
    <div class="shad-card mb-4">
        <div class="shad-card-body py-3">
            <form action="{{ route('leads.index') }}" method="GET" class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    <div class="d-flex align-items-center gap-2">
                        <label class="shad-label mb-0" for="start_date" style="white-space: nowrap;">From:</label>
                        <input type="date" name="start_date" id="start_date" class="shad-input" style="width: auto;" value="{{ $startDate }}">
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        <label class="shad-label mb-0" for="end_date" style="white-space: nowrap;">To:</label>
                        <input type="date" name="end_date" id="end_date" class="shad-input" style="width: auto;" value="{{ $endDate }}">
                    </div>
                    <div class="position-relative" id="searchWrapper">
                        <input type="text" name="search" class="shad-input" style="width: 200px;" placeholder="Search leads..." value="{{ request('search') }}">
                    </div>
                    <button type="submit" class="shad-btn shad-btn-primary shad-btn-sm">
                        <i class="fas fa-search"></i> Filter
                    </button>
                    <a href="{{ route('leads.index') }}" class="shad-btn shad-btn-ghost shad-btn-sm">Clear</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="shad-card">
        <div class="shad-card-header d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="shad-card-description">{{ $leads->total() }} total leads</p>
            </div>
        </div>
            ...
            <div class="table-responsive">
                <table class="shad-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="font-weight: 600; color: var(--slate-800);">{{ $lead->name }}</td>
                            <td>{{ $lead->email ?? '-' }}</td>
                            <td>{{ $lead->phone ?? '-' }}</td>
                            <td>{{ $lead->source ?? '-' }}</td>
                            <td>
                                ...
                                @else
                                    <span class="shad-badge shad-badge-default">{{ $lead->status }}</span>
                                @endif
                            </td>
                            ...
                            <td>
                                <div class="shad-actions justify-content-end">
                                    <a href="{{ route('leads.show', $lead->id) }}" class="shad-btn shad-btn-ghost shad-btn-sm" title="View">
                        ...
                        <tr>
                            <td colspan="8">
                                <div class="shad-empty">
                                    ...
                                    <a href="{{ route('leads.create') }}" class="shad-btn shad-btn-primary mt-3">
                                        <i class="fas fa-plus"></i> Add Lead
                                    </a>
                                </div>
                            </td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('input', function() {
        ...
        timeoutId = setTimeout(() => {
            fetch(`{{ route('leads.suggestions') }}?query=${encodeURIComponent(query)}`)
